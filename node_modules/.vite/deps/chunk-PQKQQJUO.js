...
function RemoveScrollSideCar(props) {
  ...
  var Style2 = React9.useState(styleSingleton2)[0];
...
function hasFixedPositionAncestor(element, stopNode) {
  ...
  if (parentNode === stopNode || !isElement3(parentNode) || isLastTraversableNode2(parentNode)) {
    return false;
  }
...
function getClippingElementAncestors(element, cache) {
  ...
  while (isElement3(currentNode) && !isLastTraversableNode2(currentNode)) {
...
var ColorSlider = factory((_props, ref) => {
  ...
  const getChangeValue2 = (val) => round4 ? Math.round(val * maxValue) : val * maxValue;
...
function round3(number, digits = 0, base = 10 ** digits) {
...
function rgbaToHsva({ r: r2, g, b, a }) {
  ...
  return {
    h: round3(60 * (hh < 0 ? hh + 6 : hh), 3),
    s: round3(max3 ? delta / max3 * 100 : 0, 3),
    v: round3(max3 / 255 * 100, 3),
    a
  };
}
...
function parseHexa(color) {
  const hex = color[0] === "#" ? color.slice(1) : color;
  const roundA = (a2) => round3(parseInt(a2, 16) / 255, 3);
  if (hex.length === 4) {
...
var AlphaSlider = (0, import_react197.forwardRef)((props, ref) => {
  ...
  return (0, import_jsx_runtime177.jsx)(
    ColorSlider,
    {
      ...
      ref,
      value,
      onChange: (val) => onChange == null ? void 0 : onChange(round3(val, 2)),
      onChangeEnd: (val) => onChangeEnd == null ? void 0 : onChangeEnd(round3(val, 2)),
      maxValue: 1,
      round: false,
      "data-alpha": true,
      overlays: [
...
function hsvaToHsl({ h, s, v, a }, includeAlpha) {
  ...
  return `hsla(${result.h}, ${result.s}%, ${result.l}%, ${round3(a, 2)})`;
}
...
var MenuDropdown = factory((props, ref) => {
  ...
  return (0, import_jsx_runtime225.jsxs)(
    Popover.Dropdown,
    {
      ...
      onKeyDown: handleKeyDown,
      children: [
        (0, import_jsx_runtime225.jsx)("div", { tabIndex: -1, "data-autofocus": true, "data-mantine-stop-propagation": true, style: { outline: 0 } }),
        children
      ]
    }
  );
        ...
        children: [
          (0, import_jsx_runtime247.jsx)(Combobox.DropdownTarget, { children: (0, import_jsx_runtime247.jsx)(
            PillsInput,
            {
              ...
              disabled,
              radius,
              __defaultRightSection: (0, import_jsx_runtime247.jsx)(Combobox.Chevron, { size: size4, error: error2, unstyled }),
              __clearSection: clearButton,
...
function isInRange(value, min3, max3) {
  if (value === void 0) {
    return true;
  }
  const minValid = min3 === void 0 || value >= min3;
  const maxValid = max3 === void 0 || value <= max3;
...
var NumberInput = factory((_props, ref) => {
  ...
  decrementRef.current = () => {
    ...
    let val;
    const minValue = min3 !== void 0 ? min3 : !allowNegative ? 0 : Number.MIN_SAFE_INTEGER;
    const currentValuePrecision = getDecimalPlaces2(_value);
    const stepPrecision = getDecimalPlaces2(step);
    const maxPrecision = Math.max(currentValuePrecision, stepPrecision);
    const factor = 10 ** maxPrecision;
    if (!isNumberString(_value) && typeof _value !== "number" || Number.isNaN(_value)) {
      val = clamp(startValue, minValue, max3);
    } else {
  ...
  const controls = (0, import_jsx_runtime254.jsxs)("div", { ...getStyles2("controls"), children: [
    (0, import_jsx_runtime254.jsx)(
      UnstyledButton,
      {
        ...
        tabIndex: -1,
        "aria-hidden": true,
        disabled: disabled || typeof _value === "number" && max3 !== void 0 && _value >= max3,
        mod: { direction: "up" },
        onMouseDown: (event) => event.preventDefault(),
        onPointerDown: (event) => {
          onStep(event, true);
    ...
    (0, import_jsx_runtime254.jsx)(
      UnstyledButton,
      {
        ...
        tabIndex: -1,
        "aria-hidden": true,
        disabled: disabled || typeof _value === "number" && min3 !== void 0 && _value <= min3,
        mod: { direction: "down" },
        onMouseDown: (event) => event.preventDefault(),
        onPointerDown: (event) => {
          onStep(event, false);
    ...
    InputBase,
    {
      ...
      onBlur: (event) => {
        ...
        if (clampBehavior === "blur" && typeof _value === "number") {
          const clampedValue = clamp(_value, min3, max3);
          if (clampedValue !== _value) {
            setValue(clamp(_value, min3, max3));
          }
        }
        if (trimLeadingZeroesOnBlur && typeof _value === "string" && getDecimalPlaces2(_value) < 15) {
          ...
          setValue(
            Number.isNaN(parsedValue) || parsedValue > Number.MAX_SAFE_INTEGER ? replaced : clamp(parsedValue, min3, max3)
          );
        }
      },
      isAllowed: (val) => {
        if (clampBehavior === "strict") {
          if (isAllowed) {
            return isAllowed(val) && isInRange(val.floatValue, min3, max3);
          }
          return isInRange(val.floatValue, min3, max3);
        }
        ...
        children: [
          (0, import_jsx_runtime289.jsx)(Combobox.Target, { targetType: searchable ? "input" : "button", autoComplete, children: (0, import_jsx_runtime289.jsx)(
            InputBase,
            {
              id: _id,
              ref,
              __defaultRightSection: (0, import_jsx_runtime289.jsx)(Combobox.Chevron, { size: size4, error: error2, unstyled }),
              __clearSection: clearButton,
              __clearable: clearable && !!_value && !disabled && !readOnly,
              rightSection,
              rightSectionPointerEvents: rightSectionPointerEvents || (clearButton ? "all" : "none"),
...
function getPosition({ value, min: min3, max: max3 }) {
  const position = (value - min3) / (max3 - min3) * 100;
...
// node_modules/@mantine/core/esm/components/Slider/Marks/Marks.mjs
function Marks({ marks, min: min3, max: max3, disabled, value, offset: offset4, inverted }) {
  const { getStyles: getStyles2 } = useSliderContext();
  if (!marks) {
    return null;
  }
  const items = marks.map((mark, index5) => (0, import_react324.createElement)(
    Box,
    {
      ...
      __vars: { "--mark-offset": `${getPosition({ value: mark.value, min: min3, max: max3 })}%` },
      key: index5
    },
...
function getChangeValue({
  ...
  const dx = left * (max3 - min3);
  const nextValue = (dx !== 0 ? Math.round(dx / step) * step : 0) + min3;
  const nextValueWithinStep = Math.max(nextValue, min3);
  if (precision !== void 0) {
...
var Slider = factory((_props, ref) => {
  ...
  const [_value, setValue] = useUncontrolled({
    value: typeof value === "number" ? clamp(value, min3, max3) : value,
    defaultValue: typeof defaultValue === "number" ? clamp(defaultValue, min3, max3) : defaultValue,
    finalValue: clamp(0, min3, max3),
    onChange
  });
  ...
  const position = getPosition({ value: _value, min: min3, max: max3 });
  ...
  const handleChange = (0, import_react326.useCallback)(
    ...
    [disabled, min3, max3, step, precision, setValue, marks, restrictToMarks]
  );
      ...
      switch (event.key) {
        case "ArrowUp": {
          ...
          const nextValue = getFloatingValue(
            Math.min(Math.max(_value + step, min3), max3),
            precision
          );
          setValue(nextValue);
        ...
        case "ArrowRight": {
          ...
          const nextValue = getFloatingValue(
            Math.min(Math.max(dir === "rtl" ? _value - step : _value + step, min3), max3),
            precision
          );
          setValue(nextValue);
          onChangeEnd == null ? void 0 : onChangeEnd(nextValue);
        ...
        case "Home": {
          ...
          setValue(min3);
          onChangeEnd == null ? void 0 : onChangeEnd(min3);
          break;
        }
        case "End": {
          event.preventDefault();
          ...
          setValue(max3);
          onChangeEnd == null ? void 0 : onChangeEnd(max3);
          break;
        }
      }
    }
...
var RangeSlider = factory((_props, ref) => {
  ...
  const positions = [
    getPosition({ value: _value[0], min: min3, max: max3 }),
    getPosition({ value: _value[1], min: min3, max: max3 })
  ];
  const precision = _precision ?? getPrecision(step);
  const _setValue = (val) => {
    setValue(val);
    ...
    if (restrictToMarks && marks) {
      ...
      if (index5 === 0) {
        if (val > clone[1] - (minRange - 1e-9)) {
          clone[1] = Math.min(val + minRange, max3);
        }
        if (val > (max3 - (minRange - 1e-9) || min3)) {
      ...
      if (index5 === 1) {
        if (val < clone[0] + minRange) {
          clone[0] = Math.max(val - minRange, min3);
        }
        if (val < clone[0] + minRange) {
          clone[index5] = valueRef.current[index5];
        }
    ...
    if (!disabled) {
      switch (event.key) {
        case "ArrowUp": {
          ...
          const nextValue = restrictToMarks && marks ? getNextMarkValue(valueRef.current[focusedIndex], marks) : Math.min(Math.max(valueRef.current[focusedIndex] + step, min3), max3);
        ...
        case "ArrowDown": {
          ...
          const focusedIndex = getFocusedThumbIndex();
          thumbs.current[focusedIndex].focus();
          const nextValue = restrictToMarks && marks ? getPreviousMarkValue(valueRef.current[focusedIndex], marks) : Math.min(Math.max(valueRef.current[focusedIndex] - step, min3), max3);
          setRangedValue(getFloatingValue(nextValue, precision), focusedIndex, true);
          break;
        }
        case "ArrowLeft": {
...
tabbable/dist/index.esm.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-PQKQQJUO.js.map
