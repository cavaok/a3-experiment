...
var MakeDataVisitor = class _MakeDataVisitor extends Visitor {
  ...
  visitNull(props) {
    ...
    return new Data(type, offset2, length, length);
  }
  visitBool(props) {
  ...
  visitUnion(props) {
    ...
    if (DataType.isSparseUnion(type)) {
      return new Data(type, offset2, length, nullCount, [void 0, void 0, void 0, typeIds], children);
    }
    const valueOffsets = toInt32Array(props["valueOffsets"]);
    return new Data(type, offset2, length, nullCount, [valueOffsets, void 0, void 0, typeIds], children);
  }
...
function bool_builder_default(type, length) {
  ...
  return {
    set(value, index) {
      if (value) data2[index >> 3] |= 1 << index % 8;
    },
    data: () => ({ type, length, buffers: [null, data2] })
  };
}
...
var ColumnTable = class _ColumnTable extends Table {
  ...
  static new(columns2, names) {
    if (columns2 instanceof _ColumnTable) return columns2;
    const data2 = {};
    const keys = [];
...
// node_modules/arquero/node_modules/acorn/dist/acorn.mjs
...
function arrow_dictionary_default(vector) {
  ...
  return {
    ...
    keyFor(value2) {
      ...
      for (let i = 0; i < size; ++i) {
        if (values2[i] === void 0) values2[i] = dictionary2.get(i);
        if (values2[i] === value2) return i;
      }
      return -1;
    },
    groups(names) {
...
function arrowColumn(vector, nested) {
  ...
  if (isDict(type)) return arrow_dictionary_default(vector);
...
function relocate_default(table3, columns2, { before, after } = {}) {
  ...
  table3.columnNames().forEach((name2) => {
    ...
    if (name2 === anchor) {
      if (aft && assign) select.set(name2, name2);
      for (const [key2, value] of columns2) {
        select.set(key2, value);
      }
      if (aft) return;
    }
    if (assign) select.set(name2, name2);
  });
...
function except_default(table3, others) {
  if (others.length === 0) return table3;
...
function filter_default(table3, predicate) {
  ...
  if (bits) {
    for (let i = bits.next(0); i >= 0; i = bits.next(i + 1)) {
      if (predicate(i, data2)) filter.set(i);
    }
  } else {
    for (let i = 0; i < n; ++i) {
      if (predicate(i, data2)) filter.set(i);
...
function fold_default(table3, { names = [], exprs = [], ops = [] }, options = {}) {
  if (names.length === 0) return table3;
...
function spread_default(table3, { names, exprs, ops = [] }, options = {}) {
  ...
  table3.columnNames().forEach((name2) => {
    if (map.has(name2)) {
      if (!drop) cols.add(name2, table3.column(name2));
...
function verifyTableMethod(name2, fn, options) {
  const type = "method";
  if (proto[name2] && proto[name2].fn === fn) return true;
  if (!RESERVED) {
    RESERVED = {};
    addReserved(proto);
    addReserved(Query2.prototype);
  }
  if (RESERVED[name2]) onReserve(name2, type);
  if ((name2 + "")[0] === "_") onIllegal(name2, type);
  check2(name2, options, proto, type);
}
function addTableMethod(name2, method, options = {}) {
  if (verifyTableMethod(name2, method, options)) return;
...
function addPackage(bundle, options = {}) {
  ...
  function scan2(index) {
    for (const key2 in parts) {
      ...
      const p = pkg[key2];
      for (const name2 in p) part[index](name2, p[name2], options);
    }
  }
  scan2(0);
  scan2(1);
